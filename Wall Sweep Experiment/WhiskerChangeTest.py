...
To change from straight wall to cylinderical pillar, uncomment line 37 in composite_nylon.xml
"""
...
xml_path = 'composite_nylon.xml' #xml file (assumes this is in the same folder as this file)
...
def slide_test(Whisker,speed,f_data,T_data,angle,time):

    data.ctrl[Whisker]=speed
    if(data.time<=simend and data.time % 0.05 < model.opt.timestep) :
            if(Whisker):
                f_data.append(data.sensordata[6:9].copy()) 
                T_data.append(data.sensordata[9:12].copy()) 
                angle.append(data.sensordata[12].copy())
...
def plot_force_vs_time(time,x,y,z = None):
   ...
   if z is None:
    plt.title("Force vs Time (Composite Nylon6/6 Whisker)")
   else:
    plt.title("Force vs Time (Manually Segmented Wisker)")  
   plt.xlabel("time")
   plt.tight_layout()
   plt.grid()
   plt.legend()
   ...
   plt.title("Torque vs time (Composite Nylon6/6 Whisker)")
   plt.xlabel("time")
   plt.legend()
...
def controller(model, data):
    slide_test(0,10, f_data,T_data,angle,time)
...
plot_force_vs_time(time,f_data,T_data)
